      ...
      try {
        ...
        if (_fpDF || _fpVF) {
          ...
          for (var _fpi = 0; _fpi < _fpLines.length; _fpi++) {
            ...
            if (_fpVv) { _fpEnt.nilai = parseFloat(_fpVv[1].replace(',','.')); _fpEnt.nilaiStr = _fpVv[1].replace(',','.')+' '+_fpVv[2].toUpperCase(); if(/jt/i.test(_fpVv[2])) _fpEnt.nilai /= 1000; }
              }
            }
          ...
          if (_fpRes.length > 0) {
            var _fpBody = _fpRes.slice(0,15).map(function(it,i){ return '<br><strong>'+(i+1)+'. '+it.nama+'</strong> | '+it.cat+(it.nilai!==null?' | Nilai: Rp '+(it.nilaiStr||it.nilai+' M')':'')+(it.closing?' | Closing: '+it.closing.d+'-'+it.closing.m+'-'+it.closing.y:''); }).join('<br>\n');
